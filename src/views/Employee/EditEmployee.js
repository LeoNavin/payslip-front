...
const EditEmployee = () => {
  ...
  const handleDate = (date) => {
    ...
    setStartDate(dojDate)
  }

  async function getUserProfile() {
    const admingetUserId = localStorage.getItem('admingetUserId')
    setLoading(true)

    // await axios.get('http://192.168.1.141:8088/payslip/user/getUserById?id='+admingetUserId, config).then((res) => {
    await axios.get(Base_Url + 'user/getUserById?id=' + admingetUserId, config).then((res) => {
      if (res.data.status) {
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader className="custum_header ">
            ...
            {role === 'ADMIN' ? (
              <strong>Update Employee Profile</strong>
            ) : (
